...
typedef void (*Elf_shdr_func_t)(size_t label, addr_t addr, size_t sz, unsigned access,
                                const char* name, int is_progbits_or_nobits);
typedef void (*Elf_phdr_func_t)(size_t label, addr_t vaddr, addr_t paddr, size_t sz,
...
int elf_foreach(const void* elf, size_t sz, Elf_shdr_func_t sh_func,
                Elf_phdr_func_t ph_func, size_t label, addr_t* entry, Elf_dprint_t dprint = Elf_no_dprint);
