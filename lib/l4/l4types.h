...
public:
	...
	inline bool is_single()    const { return !is_any() && !is_any_local(); }
...
public:
	...
	static inline L4_acceptor_t create(L4_fpage_t fp, bool str)
	{
		L4_acceptor_t a;
		a.set(fp, str);
		return a;
...
public:
	...
	{
		// TODO:  think
		Pfault_reply =  0,  // not unique label
		Thread_start =  0,  // not unique label, thread start proto
		Interrupt    = -1,  // unique label, interrupt proto
		Pagefault    = -2,  // unique label, pagefault proto
		Sigma0       = -6,  // unique label, sigma0 proto
	...
	union
	{
		...
		struct
		{
			enum { Label_width = sizeof(word_t) * 8 - 16 };  // 16/48 for 32/64-bit systems
			...
			unsigned untyped_words  : 6;
		} _ipc;
		struct
		{
			enum { Label_width = sizeof(word_t) * 8 - 20 };  // 12/44 for 32/64-bit systems
	...
	// API for proto msg
	inline long   proto_label()           const { return _proto.label; /*_proto.label&0x800 ? _proto.label-0x1000 : _proto.label;*/ }
	...
	{
		return ipc_label()==Pfault_reply  &&  typed()==2  &&  untyped()==0;
	}
};
...
public:
	...
	word_t _user_defined_handler;             //  3  rw
	...
	inline L4_msgtag_t&   msgtag()         const { return (L4_msgtag_t&) mr[0];   } // FIXME:  remove me !!!
